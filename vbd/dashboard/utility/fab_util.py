...
def fab_run_command(command, is_local=False, command_path=None, command_prefix=None, pty=False, warn_only=True, timeout=3, ex_abort=False):
    ...
    @param warn_only: if warn_only=True, then throw a python exception while error. If =false, fabric will throw a fatal error
    @param ex_abort: if ex_abort=True and meeting exception, then exit main process
    '''
    result = None
...
def fab_run_single_remote_command(command, host, key_filename, user='root', port=22, command_path=None, command_prefix=None, pty=False, warn_only=True, timeout=3, ex_abort=False):
    '''
    @param command: 
    @param host: remote host
    @param key_filename: remote key file
    @param command_path: command path
    @param command_prefix: the command will be 'command_prefix && command'
    @param warn_only: if warn_only=True, then throw a python exception while error. If =false, fabric will throw a fatal error.
    ...
    if result and result.failed:
        ...
        if ex_abort:
            abort("abort!!!")
        raise Exception("Failed to execute command. Error:%s" %(str(result)))
    return result
...
if __name__ == '__main__':
    ...
    key_file = '/Users/xieyanyang/work/ttbj/ttbj-keypair.pem'
    cmd = "hostname -f"
    r = fab_run_single_remote_command(cmd, host, key_file)
    print r
